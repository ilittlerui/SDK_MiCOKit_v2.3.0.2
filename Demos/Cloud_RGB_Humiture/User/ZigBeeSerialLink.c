...
static teSL_Status eSL_ReadMessage(uint16_t *pu16Type, uint16_t *pu16Length, uint16_t u16MaxLength, uint8_t *pu8Message,uint8_t *inDataBuffer,int recvlen);
...
void uartRecv_thread(void *inContext)
{
    ...
    {
        // ======================get msg from uart=======================
        recvlen = user_uartRecv(inDataBuffer, USER_UART_ONE_PACKAGE_LENGTH);
        if (recvlen <= 0)
            continue;
        /* Initialise buffer */
        ...
        if (eSL_ReadMessage(&sMessage.u16Type, &sMessage.u16Length, SL_MAX_MESSAGE_LENGTH, sMessage.au8Message,inDataBuffer,recvlen) == E_SL_OK)
        {
            user_uart_log("message get,type:%d",sMessage.u16Type);
        ...
        // if the message should send to cloud????????????
